...
## Comments: 
ggplot() + 
  geom_density(data=params700, aes(x=phi1, colour='H=Dynamic')) +
...
ggplot() + 
  geom_density(data=params700, aes(x=phi2, colour='H=Dynamic')) +
...
ggplot() + 
  geom_density(data=params700, aes(x=phi3, colour='H=Dynamic')) +
...
ggplot() + 
  geom_density(data=params700, aes(x=phi4, colour='H=Dynamic')) +
...
## Comments: 
ggplot() + 
  geom_density(data=params900, aes(x=phi1, colour='H=Dynamic')) +
...
############### GET STATISTICS ###############
statistics_df <- data.frame(matrix(ncol=7,nrow=0, dimnames=list(NULL, c("Phi", "T", "H", "min", "max", "SE", "VAR"))))

for (x in all_dfs) {
  # Add phi1 data
  phi1_data <-  data.frame(c("phi1", x[1], x[2], min(x$phi1), max(x$phi1), sd(x$phi1), var(x$phi1)))
  colnames(phi1_data) <- colnames(statistics_df)
  statistics_df <- rbind(statistics_df, phi1_data)
  
  # Add phi2 data
  phi2_data <-  data.frame(c("phi2", x[1], x[2], min(x$phi2), max(x$phi2), sd(x$phi2), var(x$phi2)))
  colnames(phi2_data) <- colnames(statistics_df)
  statistics_df <- rbind(statistics_df, phi2_data)
  
  # Add phi3 data
  phi3_data <-  data.frame(c("phi3", x[1], x[2], min(x$phi3), max(x$phi3), sd(x$phi3), var(x$phi3)))
  colnames(phi3_data) <- colnames(statistics_df)
  statistics_df <- rbind(statistics_df, phi3_data)

  # Add phi4 data
  phi4_data <-  data.frame(c("phi4", x[1], x[2], min(x$phi4), max(x$phi4), sd(x$phi4), var(x$phi4)))
