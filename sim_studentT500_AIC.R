...
for (i in 1:sims) {
  ...
  # Dynamic H
  H <- VARselect(sim$Y_sim, lag.max = 25, type="none")$selection[1]
...
# Save results to csv
write.table(params_studentT, file="results/params_studentT500_AIC.csv", sep = ',', row.names = FALSE, col.names = FALSE)
